...
description: 根据新增或指定的 API 接口生成 Markdown 格式的接口文档，输出到 ../csi-front/tmp_docs。适用于 FastAPI 路由与 Pydantic 模型。在用户新增接口后需要写接口文档、或明确要求生成接口文档时使用。
---
...
将生成的 Markdown 文档输出到**项目前端目录的 `tmp_docs` 文件夹**（即 `{项目根}/../csi-front/tmp_docs`）。若目录不存在需先创建。

文件名建议：`API-{模块或功能名}.md` 或 `API-{日期}-{功能}.md`，避免覆盖已有文档时可先确认或使用更具体的文件名。
...
3. **按下方模板生成文档**，并写入 `../csi-front/tmp_docs` 目录下的 `.md` 文件。
