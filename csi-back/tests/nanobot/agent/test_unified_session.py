...
from app.service.nanobot.command.builtin import cmd_new, register_builtin_commands
from app.service.nanobot.command.router import CommandContext, CommandRouter
from app.service.nanobot.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("app.service.nanobot.agent.loop.SessionManager"), \
         patch("app.service.nanobot.agent.loop.SubagentManager") as MockSubMgr, \
         patch("app.service.nanobot.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        ...
        from app.service.nanobot.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from app.service.nanobot.agent.memory import Consolidator, MemoryStore
