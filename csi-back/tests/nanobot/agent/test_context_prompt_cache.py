...
def test_bootstrap_files_are_backed_by_templates() -> None:
    template_dir = pkg_files("app.service.nanobot") / "templates"
...
def test_execution_rules_in_system_prompt(tmp_path) -> None:
    ...
    from app.service.nanobot.utils.helpers import sync_workspace_templates
...
def test_default_soul_template_contains_execution_rules() -> None:
    ...
    soul = (pkg_files("app.service.nanobot") / "templates" / "SOUL.md").read_text(encoding="utf-8")
    assert "## 执行规则" in soul
    assert "单步任务" in soul
    assert "多步任务" in soul
