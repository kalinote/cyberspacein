...
- 在更可靠时，优先使用 Windows 原生命令或已注册的业务工具。
- 若终端输出乱码，请启用 UTF-8 输出后重试。
{% else %}
## 平台策略（POSIX）
- 你正在 POSIX 系统上运行。请优先使用 UTF-8 与标准 shell 工具。
- 当比 shell 命令更简单或更可靠时，请使用已注册的业务工具。
{% endif %}
"""

_SKILLS_SECTION_CONTENT = """# 技能

以下技能可扩展你的能力。非 always 技能请调用 `use_skill` 工具（参数 skill_id，可选 path 加载子文件）。
...
按需要调用 `use_skill(skill_id)` 加载技能正文或子文件。
